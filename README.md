Repositorio de la materia Desarrollo de Sistemas IoT, Julian Felipe Gutierrez Ramirez, Universidad Nacional de Colombia sede Manizales.
...
**Departamento:** Departamento de Ingeniería Eléctrica, Electrónica y Computación

**Universidad:** Universidad Nacional de Colombia - Sede Manizales

---
Esta es la guía para realizar la demo de un sistema IoT básico donde un ESP32-C6 controla un LED RGB (LED interno) que cambia de colores cada 3 segundos, envía la información por MQTT a un servidor en AWS EC2, y una página web muestra los cambios de color en tiempo real.
...
#### 1.1 Acceder a AWS Console:
  - Ve a AWS Console [AWS Console](https://aws.amazon.com/console/)
...
Si aún no se tiene Apache instalado, se puede hacer usando el gestor de paquetes apt, abriendo una terminal y ejecutando estos comandos:
```bash
sudo apt update
...
Por convención, las páginas web se almacenan en el directorio **/var/www/html**. Se puede utilizar este directorio para una página web sencilla. Puedes crear un directorio para el sitio y cambiar los permisos para poder editar los archivos:
...
Se debe asegurar de que Apache esté en ejecución. Puedes verificar su estado con el siguiente comando:
...
**Importante:** El servidor predeterminado no admite HTTPS, así que asegúrate de ingresar mediante HTTP.

![Untitled](./img/21.png)

### Mosquitto Server (MQTT)

Para crear un servidor MQTT en Ubuntu, se puede utilizar el popular servidor MQTT llamado Mosquitto. Mosquitto es de código abierto y ampliamente utilizado en la comunidad de IoT y en aplicaciones que requieren comunicación de mensajes entre dispositivos. A continuación se muestra cómo instalar y configurar Mosquitto en Ubuntu:
...
Una vez que Mosquitto esté instalado, se puede habilitar el servicio y asegurarse de que se inicie automáticamente al arrancar el sistema con los siguientes comandos:
...
Para verificar si Mosquitto se está ejecutando correctamente, ejecutar el siguiente comando:

```bash
sudo systemctl status mosquitto
```

Se debería ver un mensaje que indique que el servicio está activo y en funcionamiento.
...
Realiza las modificaciones que desees y guarda el archivo. Un ejemplo de configuración es:
...
- Abrir una terminal y ejecutar el siguiente comando para crear un archivo que almacene los usuarios y contraseñas:

```bash
sudo mosquitto_passwd -c /etc/mosquitto/passwd <nombre-de-usuario>
```

Reemplazar **`<nombre-de-usuario>`** con el nombre de usuario que se desee. Será solicitado ingresar una contraseña para ese usuario.

![Untitled](./img/24.png)

Luego, asignar los permisos adecuados al archivo:
...
Abrir otra terminal y publicar un mensaje en el mismo tema:
...
Para la aplicación a desarrollar se debe colocar este código HTML en la parte web para visualización de la información. Este archivo se crea en la siguiente ruta:
